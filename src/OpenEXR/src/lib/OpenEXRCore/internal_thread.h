...
#    ifdef _WIN32
#        include <windows.h>
#        define EXRCORE_ONCE_FLAG_INIT INIT_ONCE_STATIC_INIT
typedef INIT_ONCE exrcore_once_flag;
...
static inline void
exrcore_call_once (exrcore_once_flag* flag, void (*func) (void))
{
    InitOnceExecuteOnce (flag, once_init_fn, (PVOID) func, NULL);
}
#    else
#        include <pthread.h>
#        define EXRCORE_ONCE_FLAG_INIT PTHREAD_ONCE_INIT
typedef pthread_once_t exrcore_once_flag;
static inline void
exrcore_call_once (exrcore_once_flag* flag, void (*func) (void))
{
...
#    endif
#else
#    define EXRCORE_ONCE_FLAG_INIT 0
typedef int exrcore_once_flag;
static inline void
exrcore_call_once (exrcore_once_flag* flag, void (*func) (void))
{
    if (!*flag) {
