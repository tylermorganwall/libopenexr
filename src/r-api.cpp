...
extern "C" SEXP C_read_exr(SEXP path_SEXP) {
    ...
    try {
        ...
        // Allocate matrices (column‑major in R, but we’ll fill row‑major then transpose)
        ...
        double *r = REAL(rMat), *g = REAL(gMat),
               *b = REAL(bMat), *a = REAL(aMat);
...
extern "C" SEXP C_write_exr(SEXP path_SEXP, SEXP rMat, SEXP gMat,
                             SEXP bMat, SEXP aMat,
                             SEXP w_SEXP,   SEXP h_SEXP)
{
    const char *path = CHAR(STRING_ELT(path_SEXP, 0));
    const int w = INTEGER(w_SEXP)[0];
    const int h = INTEGER(h_SEXP)[0];

    check_bool(Rf_isMatrix(rMat) && Rf_isMatrix(gMat) &&
               Rf_isMatrix(bMat) && Rf_isMatrix(aMat),
               "All channels must be matrices");
    check_bool(Rf_nrows(rMat) == h && Rf_ncols(rMat) == w, "Dimension mismatch");

    const double *r = REAL(rMat), *g = REAL(gMat),
                 *b = REAL(bMat), *a = REAL(aMat);
